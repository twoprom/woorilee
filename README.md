...
이 저장소는 일반적인 storyboard/nib 기반 AppKit 앱이 아닙니다. `woorilee/main.swift`가 `NSManualApplication.shared`를 직접 실행하고, `woorilee/AppDelegate.swift`가 `IMKServer`를 프로그램적으로 등록합니다. 입력기 런타임 구조를 바꿀 때는 이 실행 경로를 유지해야 합니다.
